...
router.get("/:id", authMiddleware, async (req, res, next) => {
  ...
  try {
    ...
    if (flashcard.subject.user.id !== user.id)
      return res.status(401).send("unauthorized to see this card");
    // Already authorized to see this card

    const alt = await Flashcard.findAll({
...
router.post("/", authMiddleware, async (req, res, next) => {
  ...
  try {
    ...
    if (!subjectIds.includes(flashcard.subjectId))
